...
# Parameter
# Fix window = 500 kb, MP
# region table chr start end
regionsep<-function(chr,length){
  start<-seq(0,	length,500000)
  end<-seq(500000,length+500000,500000)
  region<-as.data.frame(cbind(chr=chr,start,end))
  region$r0<-paste(region$chr,rownames(region),sep="_")
  return(region)
}
length0<-read.table("GRCh37.length",sep = "\t",as.is = TRUE,header=TRUE)
region<-regionsep(length0[1,1],length0[1,2])
for (i in 2:22){
  region<-rbind(region,regionsep(length0[i,1],length0[i,2]))
}
#options(scipen = 20)
#options(digits=15)
write.table(region,"GRCh37_500kb",quote=FALSE,row.names = FALSE)
